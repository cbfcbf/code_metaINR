...
% However, this paper will show the superiority of SIREN in sparse time series samples with meta-learning method.
% Inspired by these and other seminal works, this paper incorporates meta-learning to SIREN-based INR of time series and demonstates its superiority in further FDA analysis.
...
\section{Experiments}
We evaluate MetaINR on the synthetic data, with benchmark estimations provided by Pre-smoothing \& PACE methods for FDA performance, and Neural Laplace method for ML-based INR task.
...
In our experiments, we assessed MetaINR using various synthetic data distributions.
...
To facilitate reader comprehension of our method, we select the following one particular example in the main text.
Experimental results of MetaINR on other datasets can be found in the appendix \ref{Experiments}.
%在实验中，我们使用了不同的数据distribution来评价MetaINR, including 周期性数据集（sine function，）, 微分方程数据集等。为了让读者方面理解方法，我们选取了其中一个特殊的例子在正文中进行展示。MetaINR在其他数据集上的实验结果可以见附录。
...
%这些参数的选择是根据工程经验为了提高算法运行速度调试的，对于不同数据集需要根据数据集的性质在算法运行过程中进行调整
...
In the learning process, we observed the capability of meta-learning to capture commonalities among a series of similar time-series data.
...
\textbf{Strong generalization ability of MataINR}
Accurate and stable sample recovery is the foundation of further functional data analysis achieved by MetaINR.
Although there existing many other machine learning-based methods perform well in the INR task, the generalization ability of them is relatively lower than MetaINR. 
...
The difference of predictions between MetaINR and Neural Laplace method for the interval [5, 10] is shown in Figure \ref{ML_compare}.
...
% 准确且稳定的Sample recovery 是MetaINR实现进一步函数形数据分析的基础。
% 尽管很多现有的基于机器学习的方法也在这个任务中表现良好，但是却没有MetaINR这么强的泛化能力。MetaINR在学习的过程中保留了SIREN作为INR的基础架构，它通过引入恰当的模型结构，有效地减少了由于训练数据的局限性导致的偏差。
% 为了证明这一点，我们以最新的Neural Laplace方法作为benchmark, 探索不同机器学习模型面对新样本的泛化能力。在训练数据中，我们给定[0,5]时间区间中等间隔分布的10个observation作为support set；而在测试数据中，我们采取【0，5】之间不同分布或者不同个数的观测作为support set。然后我们观察MetaINR和Neural Laplace分别对【5，10】区间的预测情况，如图\ref{ML_compare}。
% It is clear that 尽管在测试数据中使用与训练数据相同时间点的support set时MetaINR的表现略逊于Neural Laplace；但是当我们更改测试数据集support set的分布和数量时，MetaINR能保持较为稳定且准确的结果，而Neural Laplace 的准确性却有显著的下降。
% 正是因为MetaINR对于不同observation情况强大的泛化能力，使得它可以在基于INR的FDA任务中相比其他ML方法有更好的表现。

\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{ML_Compare.pdf}
  \caption{\textbf{MetaINR reducing the inductive bias from data.} The figure compares the generalization ability of MetaINR to Neural Laplace method. 
  ...
  By comparing the averaged RMSE of predictions in [5,10] over 5 runs, it demonstates that MetaINR is still able to maintain stable and accurate results despite changes in the support set, while the accuracy of Neural Laplace significantly decreases.
  }
  \label{ML_compare}
\end{figure}
...
Based on MetaINR workflow, we recover samples and estimated the mean function, covariance kernel, and principal components of this synthetic dataset. 
As demonstrated in Figure \ref{Estimation for synthetic data},
the estimators based on MetaINR for all metrics significantly outperforms the traditional Pre-smoothing and PACE method. 
...
In covariance kernel estimation, the estimation of both Pre-smoothing and PACE methods also exhibit differences from the ground truth, 
...
Regarding Principal Component (PC) estimation, we find that all three methods performed well in estimating PC1. 
...
\begin{figure}[htb]
  ...
  \caption{\textbf{Estimation for synthetic data.} The figure shows the results of the sample recovery and the estimation of mean function, covariance kernel and principal components on synthetic data. 
...
% Additionallly, we intend to explore other algorithms of meta-learning, such as MANN \cite{santoro2016meta} and SNAIL \cite{mishra2017simple}, and conduct comprehensive performance comparisons.
% Such investigations would offer valuable insights into the comparative strengths and weaknesses of different meta-learning approaches, 
% thereby informing future enhancements and optimizations of MetaINR.
...
\textbf{The meta-model learning mean function} %表格
% 如Figure \ref{Learning mean functions} 所示，当我们采取不同主成分来生成分布时，我们都能够观察到元模型在学习过程中会逐渐地趋向于均值。
...
during the learning process.\begin{figure}[htb]
  ...
  \caption{\textbf{Learning mean functions.} The figure illustrates that during the learning process, the meta-models gradually converge towards the mean functions of different distributions which is generated by two different principal components.  }%图片展示了，在数据集服从不同分布下，meta-model 会随着学习的过程趋向于分布的均值函数
...
% 这是当我们把MetaINR运用在一个大多数样本不平滑且不可导分布上的例子。在这个例子中，Data distribution is given by:
...
A \thicksim Uniform[0,1], \quad B \thicksim Uniform[0,0.5].
$$
We randomly draw $n=100$ sample tasks and $J_i=20$ observations is randomly chosen in each sample.
% 从图中我们发现，MetaINR的表现依然远强于其他benchmarks。特别是在不可导的点上，它依赖SIREN的架构对这些点的细节进行了精准的刻画，而传统方法却很难recover sample在转折点处的变化。
% 另一个值得注意的点是，在主成分的估计上，MetaINR给出的结果虽然和Ground truth有较大差别，但是它却和已知样本所有信息情况下对主成分的估计非常接近。
% 可以证明的是，当样本足够多的情况下，$\widehat{PC}(t)_{sample}$会收敛于ground truth $PC(t)$。 因此，基于MetaINR的估计依然是一致的。
...
they closely approximate the sample estimation of principal components $\widehat{PC}(t)_{sample}$, which is based on all samples with full observations (not the sparse observarions).
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{sawtooth_results.pdf}
  ...
  It is noticable that the performance of MetaINR is significantly better than that of Pre-smoothing and PACE methods, especially around non-differentiable time points. }
...
% \textbf{MetaINR estimation of data distribution generated by differencial equations} %来不及可不做，原文中记得改
