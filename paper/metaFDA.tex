...
In fact, the representation of meta-function $f_\theta(\cdot)$ can be diverse. There are two main classes of methods:  Parameterized Policy Gradient Methods (PPG) and Black Box Methods \cite{beck2023survey}.
...
\section{Experiments}
We evaluate our method on the synthetic data, with benchmark estimations provided by Pre-smoothing method and the PACE method.
Our results clearly demonstrate that MetaINR is able to effectively recover the samples from sparse obervations. The estimations provided by MetaINR significantly outperforms the conventional baseline. 
...
\textbf{Parameters setting} \textcolor{red}{[TODO]}
%具体的参数设置
%recover K=2
