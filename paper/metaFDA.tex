...
In such functional spaces, the focus is not only on the values of vectors in a particular dimension but also on properties such as continuity and derivatives. 
...
In functional data analysis, one can employ the method of local polynomial regression to initially estimate the original form of functional data.
Subsequently, based on this estimation, various statistical quantities such as mean and covariance operators can be estimated (see details in the appendix).
The local polynomial regression can be represented as follows:
$$
\sum_{i=1}^n\left[Y_i-\sum_{j=0}^p \beta_j\left(X_i-x_0\right)^j\right]^2 K_h\left(X_i-x_0\right) .
$$
