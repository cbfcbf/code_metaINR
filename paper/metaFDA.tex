...
\begin{itemize}
  ...
  \item Use meta-learning framework to effectively and robustly estimate INR for each sample with sparse observations.
\end{itemize}
...
% Therefore, in such a setting, MetaINR demonstrates significant superiority.
...
\begin{figure}[htb]
  ...
  \caption{\textbf{Estimation of PBC dataset.}The figure demoofnstrates the results of the sample recovery (three typical examples), and the estimation of statistical quantities
...
during the learning process.\begin{figure}[htb]
  ...
  \caption{\textbf{Learning mean functions.} The figure illustrates that during the learning process, the meta-models gradually converge towards the mean functions of different distributions which are generated by two different principal components.  }%图片展示了，在数据集服从不同分布下，meta-model 会随着学习的过程趋向于分布的均值函数
...
they closely approximate the sample estimation of principal components $\widehat{PC}(t)_{sample}$, which is based on all samples with full observations (not the sparse observations).
