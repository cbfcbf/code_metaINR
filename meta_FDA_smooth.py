...
class sinusoid(Dataset):
    def __init__(self,total_samples_per_task):
        ...
        for _ in np.arange(Number_of_task):
            A1s=random.gauss(0.5)
            A2s=random.gauss(0.5)*0.5
            A1c=random.random()
            A2c=random.random()
...
for task in data_loader:
    ...
    y_base=localreg(t_sample, y_sample,x0=t, degree=1, kernel=rbf.gaussian, radius=0.5)
...
plt.legend()
plt.savefig("./figure/learning_mean2.pdf")
...
pc1_true_sample,pc2_true_sample=fpca_discretized_true.components_.data_matrix

pc1_true= np.ones(len(t))
pc1_true = pc1_true / np.linalg.norm(pc1_true, ord=2, keepdims=True)*10
pc2_true= - np.log(t+1)
pc2_true = pc2_true / np.linalg.norm(pc2_true, ord=2, keepdims=True)*10
...
ax3.contour(X,Y,Z3,zdir='z', offset=0,cmap="rainbow")
...
ax2.contour(X,Y,Z2,zdir='z', offset=0,cmap="rainbow")
...
ax4.contour(X0,Y0,Z4,zdir='z', offset=0,cmap="rainbow")
